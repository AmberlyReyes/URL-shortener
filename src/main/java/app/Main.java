...
public class Main {
    ...
    private static void writeStartupLog() {
        try {
            Path dir = Paths.get("/app/logs");
            Files.createDirectories(dir);
            Files.writeString(
                dir.resolve("app.log"),
                "Started at " + Instant.now() + System.lineSeparator(),
                StandardOpenOption.CREATE, StandardOpenOption.APPEND
            );
        } catch (IOException e) {
            System.err.println("No pude escribir log en /app/logs: " + e.getMessage());
        }
    }
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        // Leer variables de entorno una sola vez (eficiente)

        final int PORT = Integer.parseInt(System.getenv().getOrDefault("PORT", "7000"));
        final String MONGO_URL = System.getenv().getOrDefault("MONGO_URL", "mongodb://localhost:27017/SusLinks");

        System.out.println("Starting server on port: " + PORT);
        System.out.println(" MongoDB URL: " + MONGO_URL);
